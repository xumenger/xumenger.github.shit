...
/*加上#include<stdlib.h>之后，编译就不会出现exit、malloc和free函数的警告了*/
#include<stdlib.h>
...
void (*comfun[])() = {edit, replace, insert, delete, quit};	/*函数指针，对应处理函数*/
...
void quit()
{
	...
	//在save函数里面已经用free释放过lineptr的内存了，所以如果再在用过save之后用free释放lineptr的内存，就会出现段错误
	//for(c=0; c<last; c++)
	//	free(lineptr[c]);	/*释放内存*/
	exit(0);
}
